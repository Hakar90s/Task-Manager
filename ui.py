...
def render_new_card(tab, idx):
    ...
    c_del, c_spacer, c_move = st.columns([1,6,2], gap=\"small\")
    with c_del:
        st.button(\"❌\", key=f\"new_del_{tab}_{idx}\",
                  on_click=callbacks.delete_new,
                  args=(tab, idx), use_container_width=True)
    with c_move:
        sel_key = f\"move_new_sel_{tab}_{idx}\"
        st.selectbox(\"\", options=[t for t in TABS if t != tab],
                     key=sel_key, label_visibility=\"collapsed\")
        st.button(\"🔀\", key=f\"move_new_btn_{tab}_{idx}\",
                  on_click=callbacks.move_new,
                  args=(tab, idx, st.session_state[sel_key]),
                  use_container_width=True)
    # Body: auto-saving textarea for new
    st.text_area(
        label=\"\",
        value=\"\",
...
def render_board():
    ...
    for col, tab in zip(cols, TABS):
        with col:
            ...
            # Add New
            st.button(\"➕ Add New\", key=f\"add_new_{tab}\",
